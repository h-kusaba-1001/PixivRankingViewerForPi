<template>
  <v-dialog v-model="tweetDialog" scrollable max-width="500px">
    ...
    <v-card>
      ...
      <spinner></spinner>
      <v-card-text>
        <v-textarea v-model="tweetDraft">
        </v-textarea>
      </v-card-text>
      <v-divider></v-divider>
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="blue" primary text @click="submitBtnClicked()">Tweet!</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
  ...
  export default {
    ...
    methods: {
      async TweetDialogOpen() {
        ...
        let imgFileName = imgPath.replace('../pixiv_img/', '').replace('.jpg', '').split('_')
        // pixivの作品IDを取得
        let articleId = imgFileName[1]

        await this.axios.get('http://127.0.0.1:8000/get_pixiv_info?illust_id=' + articleId)
        .then(response => {
      ...
      async submitBtnClicked() {
        ...
        await this.axios.post('http://127.0.0.1:8000/tweet',
          {
